...
/*	/turf/open/handle_slip(mob/living/carbon/C, knockdown_amount, obj/O, lube, paralyze_amount, force_drop)
	...
	if(has_gravity(src))
		...
		return 1 */ // Yogs - Old(new?) code that kevinz all that.

/turf/open/handle_slip(mob/living/carbon/C, knockdown_amount, obj/O, lube) // Yogs - New(old?) slippy slide code
	if(C.movement_type & FLYING)
		return 0
	if(has_gravity(src))
		var/obj/buckled_obj
		...
		if(lube&SLIDE)
			...
			if(C.force_moving) //If we're already slipping extend it
				qdel(C.force_moving)
			new /datum/forced_movement(C, get_ranged_target_turf(C, olddir, 1), 1, FALSE)	//spinning would be bad for ice, fucks up the next dir
		return 1 // Yogs new end because big block so I may aswell mark both ends
